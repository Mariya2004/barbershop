...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $name = $_POST['name'];
    $reserveService = $_POST['reserveService '];
    $telefon = $_POST['telefon'];
    $reserveTime = $_POST['reserveTime'];
    $date = $_POST['date'];
    $reserveMaster = $_POST['reserveMaster'];

    // создаем переменную с содержанием письма
    $content = $name . ' оставил заявку на услугу ' . $reserveService . ' к мастеру ' . $reserveMaster . ' на дату ' . $date . ' время ' . $reserveTime . '. Его телефон: ' . $telefon;

    // Первый параметр - кому отправляем письмо, второй - тема письма, третий - содержание
    $success = mail("admin@barbershop.com", 'Запрос на запись', $content);
